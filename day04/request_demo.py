...
if __name__ == '__main__':
    ...
    # 取出 响应正文中 key message 的值
    print(login_resp_json['message'])
    print(login_resp_json)
    # 取出响应正文中的  data 的值(这个值是一个字典类型),存入data_token的变量
    data_token = login_resp_json['data']
    #取出 data_tiken 里面 key 为 tokenHead 的值
    token_head = data_token['tokenHead']
    # 取出 data_token 里面为 key 为token 的值
